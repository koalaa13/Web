...
public class UserRepositoryImpl implements UserRepository {
    ...
    public void save(User user, String passwordSha) {
        try (Connection connection = DATA_SOURCE.getConnection()) {
            try (PreparedStatement statement = connection.prepareStatement("INSERT INTO `User` (`login`, `passwordSha`, `creationTime`, `isAdmin`) VALUES (?, ?, NOW(), ?)", Statement.RETURN_GENERATED_KEYS)) {
    ...
    @Override
    public void setAdminProp(long id, boolean isAdmin) {
        try (Connection connection = DATA_SOURCE.getConnection()) {
            try (PreparedStatement statement = connection.prepareStatement("UPDATE User SET isAdmin=? WHERE id=?")) {
                statement.setBoolean(1, isAdmin);
                statement.setLong(2, id);
                try {
                    ResultSet resultSet = statement.executeQuery();
                } catch (Exception ignored) {
                    throw new RepositoryException("Unable to set admin property [id=" + id + "].");
                }
            }
        } catch (SQLException e) {
            throw new RepositoryException("Can't find User.", e);
